...
class CartService(
    ...
    fun addProductToCart(userId: Long, productList: List<AddProductToCartRequestProduct>): List<CartProduct> {
        ...
        val cart: Cart = when (cartOptional.isEmpty) {
            true -> {
                cartRepository.save(Cart(user = userRepository.findById(userId).get()))
            }
        ...
        productRepository.findProductList(
            productIdList = productMap.map { it.key }
        ).forEach { p ->
            productMap[p.id]?.let {
        ...
        return cartProductRepository.saveAll(cartProductEntityList)
    }
